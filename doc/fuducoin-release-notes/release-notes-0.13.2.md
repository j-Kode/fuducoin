Fuducoin Core version 0.13.2 is now available from:

  <https://download.fuducoin.org/fuducoin-0.13.2.1/>
...
Please report bugs using the issue tracker at github:
  ...
  <https://github.com/fuducoin-project/fuducoin/issues>
...
updates will be released anymore. Without security updates, using a fuducoin
...
ECDSA signatures inside Fuducoin transactions now use validation using
...
Previous versions of Fuducoin Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of Fuducoin Core was to raise the default minimum
relay fee.

Fuducoin Core 0.13.2 will have a strict maximum size on the mempool. The
...
Fuducoin Core 0.13.2 also introduces new default policy limits on the length and
...
Fuducoin Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. Fuducoin Core will relay transactions with
...
entirely in Fuducoin Core 0.13, and it is at this time undecided whether the
...
Fuducoin Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
Fuducoin Core automatically creates a hidden service to listen on, without
manual configuration. Fuducoin Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if Fuducoin Core is listening, and
...
Fuducoind can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals Fuducoin
Core to use floating fees. By default, Fuducoin Core will use floating
fees.
...
(default: `0.0002` FUDU/kB).

At all times, Fuducoin Core will cap fees at `-maxtxfee=<x>` (default:
0.10) FUDU.
Furthermore, Fuducoin Core will never create transactions paying less than
...
`fuducoin.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
- Adding `dbcache=100` in fuducoin.conf
...
fuducoin-cli: arguments privacy
------------------------------
...
For example:
    ...
    $ src/fuducoin-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The Fuducoin Core code base has
...
- `fuducoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `fuducoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
for a new opcode in the Fuducoin scripting system that in combination with
...
Fuducoin transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of Fuducoin Core.
...
The two phases are distinct in the Fuducoin-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming Fuducoin wallet
  software more difficult and which seriously complicates the design of smart
  contracts for Fuducoin.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each Fuducoin block need to be stored by nodes in order to validate future
...
- **Increased security for multisig:** Fuducoin addresses (both P2PKH addresses
  ...
  believed to be provided by Fuducoin's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of Fuducoin's current ledger (called the UTXO set),
  ...
  note that Fuducoin Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  Fuducoin users to individually opt-in to almost any change in the Fuducoin
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and Fuducoin Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the Fuducoin
client to Fuducoin Core 0.13.2 to signal for segwit support. When a
...
Since Fuducoin Core 0.10.0, nodes have defaulted to only relaying and
...
- `fuducoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `fuducoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
